...
async function updateRecord(endpoint, id, name) {
    ...
    if (endpoint === 'patients') {
        ...
        if (!val) return;
        payload = { status: val };
    } else if (endpoint === 'medicines') {
        const val = prompt(`Update Stock Quantity for this Medicine:`);
        if (!val) return;
        payload = { stockQuantity: parseInt(val) };
    }
        ...
        json.data.forEach(item => {
            if (endpoint === 'patients') {
                ...
                html += `<tr>
                    ...
                    <td>LKR ${item.pricePerUnit}</td>
                    <td>${item.supplier}</td>
                    <td style="min-width: 100px;">
                        <button class="btn-edit" onclick="updateRecord('medicines', '${item._id}', '${item.medicineName}')"><i class="fas fa-edit"></i></button>
                        <button class="btn-del" onclick="deleteRecord('medicines', '${item._id}', '${item.medicineName}')"><i class="fas fa-trash"></i></button>
                    </td>
                </tr>`;
            }
        });
...
// Medicine Add (Testing CRUD for 5th member)
async function createMedicine() {
    const payload = {
        medicineName: document.getElementById('mName').value,
        stockQuantity: parseInt(document.getElementById('mQty').value),
        pricePerUnit: parseInt(document.getElementById('mPrice').value),
        supplier: document.getElementById('mSupplier').value
    };

    if (!payload.medicineName || !payload.supplier) return alert("Fill all medicine input boxes!");
    ...
    try {
        ...
        if (res.ok) {
            alert('Medicine added seamlessly using POST -> Gateway -> M5!');
